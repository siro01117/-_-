...
const SUPABASE_KEY = 'YOUR_ANON_KEY_HERE'; // 본인의 ANON_KEY 주입
...
const App = () => {
    ...
    const [isAuth, setIsAuth] = useState(false); // Auth.js에서 제어됨
                ...
                {current ? (
                    <>
                        <header className="h-24 border-b border-slate-100 flex items-center justify-between px-10 bg-white z-10 shadow-sm">
                            <h2 className="text-2xl font-black italic uppercase tracking-tight text-slate-800">{current.name} 관리 시스템</h2>
                            <div className="flex items-center gap-4">
                                {/* Export.js 모듈 호출 */}
                                <button onClick={() => window.ExportSystem.generate(captureRef.current, current.name, 'png')} className="px-6 py-3 bg-emerald-500 text-white rounded-xl font-black shadow-lg uppercase">PNG</button>
                                <button onClick={() => window.ExportSystem.generate(captureRef.current, current.name, 'pdf')} className="px-6 py-3 bg-blue-600 text-white rounded-xl font-black shadow-lg uppercase">PDF</button>
                        ...
                        <div className="flex-1 overflow-auto bg-slate-50/50 p-12 custom-scrollbar">
                            <div ref={captureRef} className="export-area bg-white p-16 rounded-[4rem] shadow-2xl border border-slate-100 mx-auto">
                                ...
                                <div className="flex gap-12">
                                    {/* 그리드 렌더링 엔진 */}
                                    <div className="flex-1 grid-layout divide-x-2 divide-slate-100 border-2 border-slate-100 rounded-[3.5rem] overflow-hidden shadow-2xl bg-white relative">
                                                    ...
                                                    {current.schedules.filter(s => s.days.includes(day)).map(s => {
                                                        ...
                                                        return (
                                                            <div key={s.id} onClick={() => isEditMode && setScheduleModal({ open: true, editId: s.id })}
                                                                className={`absolute left-1 right-1 rounded-2xl p-4 shadow-xl border border-black/5 flex flex-col overflow-hidden transition-all hover:scale-105 cursor-pointer`}
                        ...
                        {isEditMode && (
                            <button onClick={() => setScheduleModal({ open: true, editId: null })} 
                                className="absolute bottom-12 right-12 w-24 h-24 bg-blue-600 text-white rounded-full shadow-2xl flex items-center justify-center active:scale-90 transition-all z-20">
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-slate-200">
                        <lucide.icons.MousePointer2 size={120} className="mb-10 opacity-10" />
